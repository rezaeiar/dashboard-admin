...
import { getAllTasks } from "../../api/services/task"
...
const TodoList = () => {
    ...
    const { data: allTasksData } = useQuery("tasks", getAllTasks)    
                    ...
                    <div className="flex flex-col gap-y-2">
                        {
                            !!allTasksData && allTasksData.map((task : any) => (
                                <div key={task.id} className="flex justify-between bg-white/80 py-5 rounded-lg border border-general-50 px-6">
                                    <div className="flex gap-x-2 items-center">
                                        <label className="relative flex items-center p-3 rounded-full cursor-pointer" htmlFor="link">
