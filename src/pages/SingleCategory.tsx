...
import { useDeleteCategory, useDeleteProductFromCategory, useSingeCategory } from "../hooks/api/useCategories"
import { usePutCategory } from "../hooks/api/useCategories"
...
const SingleCategory = () => {
    ...
    const { data, isSuccess, isLoading } = useSingeCategory(params.id as string)
    const { mutate: editCategory } = usePutCategory(params.id as string)
    const { mutate: deleteCategory } = useDeleteCategory()
    const { mutate: deleteProductFromCategory } = useDeleteProductFromCategory(params.id as string)
    ...
    const changeCategoryInfoHandler = () => {
        ...
        if (image) categoryInfo.image = image
    ...
    const deleteProductFromCategoryHandler = (productId: string) => {
        deleteProductFromCategory(productId)
    }

    const showEditConfirmModal = () => {
        dispatch(showConfirmModal({ visibility: true, payload: { title: t("Change information"), description: t("You are changing the information of this category, are you sure?") }, button: "Continue", handler: () => changeCategoryInfoHandler() }))
    }
    const showDeleteConfirmModal = () => {
        dispatch(showConfirmModal({ visibility: true, payload: { title: t("Delete Category"), description: t("By deleting a category, all the products of this category will be deleted, are you sure?") }, button: "Delete", handler: () => deleteCategoryHandler(params.id as string) }))
    }
    const showDeleteProductConfirmModal = (productId: string) => {
        dispatch(showConfirmModal({ visibility: true, payload: { title: t("Delete product from category"), description: t("You are removing the product from this category, are you sure?") }, button: "Delete", handler: () => deleteProductFromCategoryHandler(productId) }))
    }
                                    ...
                                    data.products.map((product: { name: string, id: string, images: string[] }) => (
                                        <div className="border border-general-50 rounded-md p-2 sm:p-3 xl:p-4 flex justify-between items-center" key={product.id}>
                                            ...
                                            <div className="flex gap-x-1 sm:gap-x-3 xl:gap-x-4 text-general-70 child:cursor-pointer child:transition-colors">
                                                <Link to={`/panel/products/edit/${product.id}`}>
                            ...
                            </label>
                            {
                                image ?
                                    <label className="border cursor-pointer border-dotted border-general-60 rounded-md flex items-center justify-center aspect-[4/3] overflow-hidden p-0.5" htmlFor="small_size">
                                        <img src={image} alt="" className="w-full h-full object-cover brightness-75 rounded" />
                                    </label> :
