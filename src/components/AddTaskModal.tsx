...
const AddTaskModal = ({ isShowAddTaskModal }: AddTaskModalProps) => {
    ...
    return (
        <div className={`parent backdrop-blur-sm flex items-center justify-center fixed transition-all h-screen w-full top-0 left-0 bg-general-100/50 z-50 px-4 sm:px-6 md:px-8 ${isShowAddTaskModal ? "opacity-100 visible" : "opacity-0 invisible"}`} onClick={(event) => hideModalHandler(event)} onKeyDown={e => e.keyCode === 13 && addCategoryHandler()}>
            <div className="flex flex-col gap-y-4 bg-white w-[500px] rounded p-6">
                ...
                <div className="flex flex-col">
                    ...
                    <input type="number" className="border border-general-50 outline-none rounded text-xs sm:text-sm text-general-70 py-2 px-4 md:px-2.5 lg:px-4 font-iransans-regular placeholder:ltr:font-nunitosans-regular" placeholder={t("30 Days")} value={expire_time} onChange={e => setExpire_time(e.target.value)} />
                </div>
